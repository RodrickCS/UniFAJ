...
def obter_proximo_id():
    try:
        with open("./APSI/usuarios.csv", "r", encoding="UTF8") as f:
            reader = csv.reader(f)
...
def escrever_usuario_csv():
    try:
        with open("./APSI/usuarios.csv", "a", encoding="UTF8", newline="") as f:
            writer = csv.writer(f)
            ...
            if proximo_id is not None:
                ...
                print("Usuário escrito com sucesso no arquivo 'usuarios.csv' com ID:", proximo_id)
            else:
...
def pesquisar_usuario():
    try:
        ...
        with open("./APSI/usuarios.csv", "r", encoding="UTF8") as f:
            ...
            for row in reader:
                if len(row) >= 3:
                    if campo_pesquisa == "Nome" and termo_pesquisa.lower() == row[0].lower():
                        print(f"Usuário encontrado: Código: {row[1]}, Nome: {row[0]}, Email: {row[2]}")
                        encontrado = True
            ...
            if not encontrado:
                print(f"Nenhum usuário encontrado com {campo_pesquisa}: {termo_pesquisa}.")
    except Exception as e:
        print("Ocorreu um erro:", str(e))
...
termo_pesquisa_label.grid(column=0, row=4)
...
campo_pesquisa_label.grid(column=0, row=5)
